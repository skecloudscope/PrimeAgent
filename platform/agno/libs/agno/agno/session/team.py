...
class TeamSession:
    ...
    def get_messages(
        ...
        if skip_member_messages:
            ...
            session_runs = [run for run in session_runs if not getattr(run, "parent_run_id", None)]  # type: ignore
    ...
    def get_team_history(self, num_runs: Optional[int] = None) -> List[Tuple[str, str]]:
        ...
        completed_runs = [
            run for run in self.runs if run.status == RunStatus.completed and not getattr(run, "parent_run_id", None)
        ]
